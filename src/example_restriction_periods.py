...
for _, row in df.iterrows():
    if row["pattern_type"] == "repeating":
        ...
        tau, dose, datetime_end = cpat.get_restriction(
            cfit, row["dose_constraint_corrected"], admin_datetime, next_element=True
        )
...
df[["name", "restriction period (h)", "dose (mSv)"]].to_csv(
    "restrictions_2nd_Zr_mAb.csv", index=False
)
