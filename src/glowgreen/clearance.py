...
class Clearance_1m:
    ...
    def at_timedelta(self, timedelta, init=None):
        ...
        if self.model == 'exponential':
            ...
            return _biexp(timedelta, params[0], params[1], np.log(2) / params[2], np.log(2) / params[3])
    ...
    def get_timedelta(self, val, init=None):
        ...
        if val >= params[0]:
            raise ValueError('val greater than or equal to initial value of clearance function')
        ...
        while True:
            if self.model == 'exponential':
                ...
                hrs = fsolve(_biexp_root, hrs0, 
                    args=(params[0], params[1], np.log(2) / params[2], np.log(2) / params[3], val))[0]
    ...
    def residence_time(self):
        ...
        if self.model == 'exponential':
            ...
            return ((fraction1 * self.model_params[2]) + ((1. - fraction1) * self.model_params[3])) / np.log(2)