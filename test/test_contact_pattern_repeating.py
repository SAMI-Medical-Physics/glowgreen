...
def test_graduate_pattern():
	...
	assert np.array_equal(cpat.c, np.array([0.7, 1., 1.]))
	assert np.array_equal(cpat.d, np.array([1., 2., 2.]))
	...
	assert np.allclose(cpat.c, np.array([0.7, 1., 0.1]))
	...
	assert np.allclose(cpat.c, np.array([0.3, 1., 1., 1., 0.05, 1, 0.31]))
	assert isinstance(cpat.c, np.ndarray)
	...
	assert np.array_equal(cpat.d, np.array([1.]))
...
def test_get_dose_exp_3():
	...
	factor1 = dose_rate_init_1m / lmbda * np.exp(- lmbda * tau) / (1 - np.exp(- lmbda * 24.))
	...
	dose_hand = factor1 * factor2 / 1000.  # uSv -> mSv
	...
	assert cpat.get_dose(cfit, 1e9, datetime_admin)[0] == 0.

	with pytest.raises(ValueError):
		cpat.get_dose_finite(cfit, 2., 1.9, datetime_admin)
...
def test_get_dose_biexp_3():
	...
	# first exp
	first_factor1 = dose_rate_init_1m * fraction1 / lmbda1 * np.exp(- lmbda1 * tau) / (1 - np.exp(- lmbda1 * 24.))
	first_factor2 = 1 / (0.3)**(1.5) * np.exp(-lmbda1* 15.75) * (1- np.exp(-lmbda1 * 2))
	first_factor2 += 1 / (1.5)**(1.5) * np.exp(-lmbda1* 23.75) * (1- np.exp(-lmbda1 * 1))
	first_factor2 += 1 / (1.5)**(1.5) * np.exp(-lmbda1* 0.75) * (1- np.exp(-lmbda1 * 2.5))

	# second exp
	second_factor1 = dose_rate_init_1m * fraction2 / lmbda2 * np.exp(- lmbda2 * tau) / (1 - np.exp(- lmbda2 * 24.))
	second_factor2 = 1 / (0.3)**(1.5) * np.exp(-lmbda2* 15.75) * (1- np.exp(-lmbda2 * 2))
	second_factor2 += 1 / (1.5)**(1.5) * np.exp(-lmbda2* 23.75) * (1- np.exp(-lmbda2 * 1))
	second_factor2 += 1 / (1.5)**(1.5) * np.exp(-lmbda2* 0.75) * (1- np.exp(-lmbda2 * 2.5))

	dose_hand = ((first_factor1 * first_factor2) + (second_factor1 * second_factor2)) / 1000.  # uSv -> mSv
...
def test_get_restriction_exp():
	...
	tau, dose, tau_arr, dose_arr, datetime_end = cpat._get_restriction_arrays(cfit, dose_constraint, datetime_admin, next_element=False)
	...
	tau_fast, dose_fast, datetime_end_fast = cpat.get_restriction(cfit, dose_constraint, datetime_admin, next_element=False)
	...
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=False)[0] == 0.
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=False)[1] == cpat.get_dose(cfit, 0., datetime_admin)[0]
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=False)[4] == datetime_admin
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=True)[0] == 2.75
	assert np.isclose(cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=True)[1], cpat.get_dose(cfit, 2.75, datetime_admin)[0]) 
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=True)[4] == datetime_admin + timedelta(hours=2.75)

	# same for get_restriction
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=False)[0] == 0.
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=False)[1] == cpat.get_dose(cfit, 0., datetime_admin)[0]
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=False)[2] == datetime_admin
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=True)[0] == 2.75
	assert np.isclose(cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=True)[1], cpat.get_dose(cfit, 2.75, datetime_admin)[0]) 
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=True)[2] == datetime_admin + timedelta(hours=2.75)

	with pytest.raises(ValueError):
		cpat._get_restriction_arrays(cfit, 0., datetime_admin)
	with pytest.raises(ValueError):
		cpat._get_restriction_arrays(cfit, -1.3, datetime_admin)
	with pytest.raises(ValueError):
		cpat.get_restriction(cfit, 0., datetime_admin)
	with pytest.raises(ValueError):
...
def test_get_restriction_biexp():
	...
	# next_element True 
	tau_next, dose_next, _, _, datetime_end_next = cpat._get_restriction_arrays(cfit, dose_constraint, datetime_admin, next_element=True)
	...
	tau_fast_next, dose_fast_next, datetime_end_fast_next = cpat.get_restriction(cfit, dose_constraint, datetime_admin, next_element=True)
	...
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=False)[0] == 0.75
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=False)[1] == cpat.get_dose(cfit, 0.75, datetime_admin)[0]
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=False)[4] == datetime_admin + timedelta(hours=0.75)
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=True)[0] in [0.75 + 5 + 19/60, 0.75 + 5 + 1/3]
	assert np.isclose(cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=True)[1], cpat.get_dose(cfit, 0.75 + 5 + 1/3, datetime_admin)[0]) 
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin, next_element=True)[4] in [datetime_admin + timedelta(hours=0.75 + 5 + 19/60), datetime_admin + timedelta(hours=0.75 + 5 + 1/3)]

	# same for get_restriction
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=False)[0] == 0.75
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=False)[1] == cpat.get_dose(cfit, 0.75, datetime_admin)[0]
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=False)[2] == datetime_admin + timedelta(hours=0.75)
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=True)[0] in [0.75 + 5 + 19/60, 0.75 + 5 + 1/3]
	assert np.isclose(cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=True)[1], cpat.get_dose(cfit, 0.75 + 5 + 1/3, datetime_admin)[0]) 
	assert cpat.get_restriction(cfit, np.inf, datetime_admin, next_element=True)[2] in [datetime_admin + timedelta(hours=0.75 + 5 + 19/60), datetime_admin + timedelta(hours=0.75 + 5 + 1/3)]