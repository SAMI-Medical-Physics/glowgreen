...
def test_get_dose_exp():
	...
	assert cpat.get_dose(cfit, np.inf) == 0.
...
def test_get_dose_biexp():
	...
	first_factor2 = 1 / (0.3)**(1.5) * np.exp(-lmbda1* 0) * (1- np.exp(-lmbda1 * 6.5))
	first_factor2 += 1 / (1.5)**(1.5) * np.exp(-lmbda1* 9.5) * (1- np.exp(-lmbda1 * 24))

	second_factor1 = dose_rate_init_1m * fraction2 / lmbda2 * np.exp(- lmbda2 * tau) 
	second_factor2 = 1 / (0.3)**(1.5) * np.exp(-lmbda2* 0) * (1- np.exp(-lmbda2 * 6.5))
	second_factor2 += 1 / (1.5)**(1.5) * np.exp(-lmbda2* 9.5) * (1- np.exp(-lmbda2 * 24))
...
def test_get_restriction_exp():
	...
	* ContactPatternOnceoff._get_restriction_arrays returns ceil(true restriction period)
	...
	tau, dose, tau_arr, dose_arr, datetime_end = cpat._get_restriction_arrays(cfit, dose_constraint, admin_datetime = datetime_admin)
	tau, dose, tau_arr, dose_arr, datetime_end = cpat._get_restriction_arrays(cfit, dose_constraint, datetime_admin)
	...
	tau_fast, dose_fast, datetime_end_fast = cpat.get_restriction(cfit, dose_constraint, admin_datetime=datetime_admin)
	tau_fast, dose_fast, datetime_end_fast = cpat.get_restriction(cfit, dose_constraint, datetime_admin)
	...
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin)[0] == 0.
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin)[1] == cpat.get_dose(cfit, 0.) 
	assert cpat._get_restriction_arrays(cfit, np.inf, datetime_admin)[4] == datetime_admin
	assert cpat.get_restriction(cfit, np.inf, admin_datetime=datetime_admin)[0] == 0.
	assert cpat.get_restriction(cfit, np.inf, admin_datetime=datetime_admin)[1] == cpat.get_dose(cfit, 0.) 
	assert cpat.get_restriction(cfit, np.inf, admin_datetime=datetime_admin)[2] == datetime_admin
	...
	with pytest.raises(ValueError):
		cpat.get_restriction(cfit, 0., admin_datetime=datetime_admin)
	with pytest.raises(ValueError):
		cpat.get_restriction(cfit, -1.3, admin_datetime=datetime_admin)
	with pytest.raises(ValueError):
		cpat._exact_restriction_period(cfit, 0.)
	with pytest.raises(ValueError):
...
def test__get_restriction_arrays_biexp():
	...
	* ContactPatternOnceoff._get_restriction_arrays returns ceil(true restriction period)
	...
	first_factor1 = dose_rate_init_1m * fraction1 / lmbda1 * np.exp(- lmbda1 * tau_original) 
	first_factor2 = 1 / (0.3)**(1.5) * np.exp(-lmbda1* 0) * (1- np.exp(-lmbda1 * 6.5))
	first_factor2 += 1 / (1.5)**(1.5) * np.exp(-lmbda1* 9.5) * (1- np.exp(-lmbda1 * 24))

	second_factor1 = dose_rate_init_1m * fraction2 / lmbda2 * np.exp(- lmbda2 * tau_original) 
	second_factor2 = 1 / (0.3)**(1.5) * np.exp(-lmbda2* 0) * (1- np.exp(-lmbda2 * 6.5))
	second_factor2 += 1 / (1.5)**(1.5) * np.exp(-lmbda2* 9.5) * (1- np.exp(-lmbda2 * 24))
